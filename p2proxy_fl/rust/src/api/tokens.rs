...
impl UserDefinedKey {
    ...
    pub fn try_new(raw_hex: &str, name: Option<String>) -> Result<Self, String> {
        ...
        log::warn!("hex: {raw_hex}, key_len: {}", privk_bytes.len());
        let privk_bytes: [u8; 32] = privk_bytes.as_slice().try_into().map_err(|_| {
            format!(
                "invalid private key hex={raw_hex}, unexpected length: {}",
                privk_bytes.len()
            )
        })?;
